...
[@genType]
let rec render =
        (~context=Default, ~key: option(string)=?, doc: t): React.element => {
  ...
  | Text(text) =>
    let setting =
      ...
      | Some(marks) =>
        ...
        let className =
          Belt.Array.reduce(marks, "", (acc, mark) => {
    ...
    switch (setting) {
    | (className, Some(link)) =>
      <a ?key href={link.attrs.href} target={link.attrs.target} ?className>
        text.text->s
      </a>
